import * as PIXI from '../../demos/js/pixi.mjs'
import * as p2 from '../../dist/p2-es.js'
import * as dat from '../../demos/js/dat.gui.module.js'
...
export class Demo extends p2.EventEmitter {
    ...
    init() {
        ...
        // if (el.addEventListener) {
            el.addEventListener('wheel', MouseWheelHandler, false) // IE9, Chrome, Safari, Opera
    ...
    render() {
        ...
        if (this.selected.length) {
            ...
            let offset=2/this.container.scale._x
            g.lineStyle(offset, 0x336622, 1, 1, false)

            offset*=5
            for (let b of this.selected) {
                const aabb = b.getAABB()
                g.drawRect(
                    aabb.lowerBound[0]-offset,
                    aabb.lowerBound[1]-offset,
                    aabb.upperBound[0] - aabb.lowerBound[0] +2*offset,
                    aabb.upperBound[1] - aabb.lowerBound[1] +2*offset
                )
            }
            this.selectionGraphics.drawn=true
        }else if (this.selectionGraphics.drawn){
            this.selectionGraphics.drawn=false
            this.selectionGraphics.clear()
        }
    ...
    getIslandColor(body) {
        ...
        if (body.islandId === -1) {
            return 0x444444 // Gray for static objects
        }
    ...
    handleMouseDown(physicsPosition) {
        switch (this.state) {
            case Demo.DEFAULT:
                ...
                if (b) {
                    this.selected.includes(b) || this.selected.push(b)
